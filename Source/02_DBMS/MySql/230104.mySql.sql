...
-- 데이터베이스로 들어감
show databases;
create database ydb; -- 새로운 데이터베이스 생성
use ydb; -- ydb database로 들어감
...
select * from student;
select * from major;
insert into major (mName, mOffice) values ('컴퓨터공학','203호'); -- 컴공번호 1
...
insert into student values (1113, '엄기봉', 4);
select * from student;
...
insert into student values (1111, '홍길동', 1);
insert into student values (1112, '신길동', 3);
insert into student values (1113, '엄기봉', 4); -- 에러
select * from student;
...
select p.dno, dname, pay from personal p, division d
	where d.dno=p.dno and pay =(select max(pay) from personal);
...
select p.pname, p.pay, p.dno, round((select avg(pay) from personal where p.dno=dno)) avg
	from personal p
...
-- 19. 이름, 급여, 해당부서평균
select pname, pay, (select avg(pay) from personal where p.dno=dno) from personal p;

-- 20. 이름, 급여, 부서명, 해당부서평균
select pname, pay, dname, (select avg(pay) from personal where p.dno=dno) from personal p, division d
	where p.dno=d.dno;
...
select now(); 
select year(sysdate()), month(now()), day(now()), hour(now()), minute(now()), second(now());
select weekday(now());
...
-- date_format(날짜, 패턴) => 문자
-- date_format(문자, 패턴) => 날짜
	-- 포맷 : %Y 2023(년도4자리) %y 23(년도2자리)
    -- 		 %m (01월, 02월,...) %c(1,2,....) %M 월이름(January, February,...), %b 짧은 월이름(Jan, Feb,...)
    -- 		 %d (01, 02,...) %e(1, 2, 3...)
...
-- format(숫자, 소수점자리수) -> 문자
select pname, format(pay, 2) from personal; -- 소수점 2자리까지, 세자리마다 ,
select pname, format(pay, 0) from personal; -- 소수점 2자리까지, 세자리마다 ,

-- 이름, 급여, 급여 3000이상인지 여부
select pname, pay, if(pay>=3000, '이상', '이하') payOver3000 from personal;
select pname, pay, bonus, if(bonus is null, 0, bonus) from personal;

-- ★ ★ ★ top-n 구문(rownum이 없음) => (limit)
select pname, pay from personal order by pay desc;
-- limit n (1~ n등)
select pname, pay from personal order by pay desc limit 5;
-- limit n1, n2 (n1번째부터 n2개) 첫번째는 0번째=> n1=0
...
select pname, pay from personal order by pay desc limit 7, 3; -- 6등부터 5개