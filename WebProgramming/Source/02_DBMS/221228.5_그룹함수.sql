...
SELECT ENAME, MAX(SAL) FROM EMP; -- 최대 급여를 받는 사람의 이름과 최대 급여 -> VI. 서브쿼리
...
SELECT TO_CHAR(MIN(HIREDATE),'RR,MM,DD')||':'|| TRUNC(SYSDATE-MIN(HIREDATE))||'일째 ' FIRST, 
    TO_CHAR(MAX(HIREDATE),'RR,MM,DD')||':'|| TRUNC(SYSDATE-MAX(HIREDATE))||'일째' LAST
    FROM EMP;

-- 탄탄ex3.  (결과) 80년12월17일 최초입사:14,620일째 83년01월12일 최근입사 :13,864일째
SELECT TO_CHAR(MIN(HIREDATE),'RR"년"MM"월"DD"일"')||' 최초입사:'|| TRIM(TO_CHAR(TRUNC(SYSDATE-MIN(HIREDATE)),'99,999'))||'일째' FIRST, 
    TO_CHAR(MAX(HIREDATE),'RR"년"MM"월"DD"일"')||' 최근입사:'|| TRIM(TO_CHAR(TRUNC(SYSDATE-MAX(HIREDATE)), '99,999'))||'일째' LAST
...
SELECT DEPTNO, MAX(SAL), AVG(SAL) FROM EMP GROUP BY DEPTNO ORDER BY DEPTNO;
    -- GROUP BY 절에서는 컬럼의 별칭을 사용할 수 없다(ORDER BY절에서만 컬럼별칭 사용 가능)
SELECT DEPTNO 부서, MAX(SAL), AVG(SAL) FROM EMP GROUP BY DEPTNO ORDER BY 부서;
--              3                       1               2               4

    -- 부서 이름별 최대 급여, 평균 급여( 평균 급여순 정렬)
SELECT DNAME, MAX(SAL), AVG(SAL) AVG_SAL
    ...
    -- ex. SAL이 5000 미만인 사원에 대해서만 부서번호별 사원수, 최대급여, 최소급여, 평균급여(부서번호 순 정렬)
...
-- ★ 3. HAVING 절 : 그룹함수의 결과가 조건으로 들어갈 때
...
-- 5. 부서번호별, 업무별 그룹하여 결과를 부서번호, 업무, 인원수, 급여의 평균, 급여의 합을 출력(부서번호, 업무순으로 오름차순 정렬)
...
--  1983     1
--  total    14	
SELECT NVL(TO_CHAR(HIREDATE, 'YYYY'), '총합'), COUNT(*)
    FROM EMP
