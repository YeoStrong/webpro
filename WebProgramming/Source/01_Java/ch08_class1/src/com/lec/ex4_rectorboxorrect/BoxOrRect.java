package com.lec.ex4_rectorboxorrect;
// 데이터 : width, height, depth, volume
	// 사각형: width, height, depth=0, volume(넓이) : BoxOrRect r = new BoxOrRect(2, 3);
	// 직육면체: width, height, depth, volume(부피)	: BoxOrRect b = new BoxOrRect(2, 3, 4);
...
public class BoxOrRect {
	...
	public void infoPrint() {
		if(depth==0) {
			...
			System.out.printf("직육면체 - 가로 : %d, 세로: %d, 넓이: %d, 부피: %d\n", width, height, depth, volume);
		}
	}
	public String infoString() {
		if(depth==0) {
			...
			return String.format("직육면체 - 가로 : %d, 세로: %d, 넓이: %d, 부피: %d", width, height, depth, volume);
		}
	}
	public int getWidth() {
		return width;
