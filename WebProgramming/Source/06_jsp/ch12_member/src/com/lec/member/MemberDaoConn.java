...
public class MemberDaoConn {
	...
	public static final int MEMBER_NONEXISTENT = 1; // 생성 가능한 ID일 때 리턴값
...
//	private MemberDaoConn() {}
	...
	private Connection getConnection() throws SQLException {
		...
		// 커넥션 풀의 DateSource 안의 conn 객체를 return
		try {
	...
	public int joinMember(MemberDto dto) {
		...
		try {
			...
			System.out.println(result==SUCCESS? "회원가입 성공" : "회원가입실패"); // 회원가입실패는 catch로 빠짐
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			System.out.println(e.getMessage());
	...
	public int loginCheck(String id, String pw) {
		...
		try {
			...
			if(rs.next()) {
				...
				// 틀린 id
				result = LOGIN_FAIL_ID; // 어차피 위에서 fail로 초기화 돼있어서 꼭 필요하지는 않음. 가독성용
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			System.out.println(e.getMessage());
	...
	public int modifyMember(MemberDto dto) {
		...
		try {
			...
			result = pstmt.executeUpdate();
			System.out.println(result==SUCCESS? "회원정보 수정 성공" : "회원정보 수정 실패(id("+dto.getId()+")가 없음)"); // 회원가입실패는 catch로 빠짐
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			System.out.println(e.getMessage());
