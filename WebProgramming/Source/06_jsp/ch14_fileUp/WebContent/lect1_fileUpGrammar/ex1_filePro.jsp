...
<body>
	<%
		String path = request.getRealPath("fileUpFolder"); // 첨부파일이 저장될 서버(WAS) 폴더
		out.print(path);
		int maxSize = 1024*1024*10; // 업로드 최대 용량(10MB)
		String filename = "";
		String originalFilename = "";
		try{
			MultipartRequest mRequest = new MultipartRequest(request, path, maxSize, "utf-8", new DefaultFileRenamePolicy()); // request객체, 저장될 서버 경로, 파일 최대 크기, 인코딩 방식, 같은 이름의 파일명 방지 처리			
			//new DefaultFileRenamePolicy() : 같은 이름의 파일이 있을 경우 = 1.jpg -> 11.jpg -> 12.jpg ... autoRename해줌 
		...
		try{
			...
			if(serverFile.exists()){
				...
				os = new FileOutputStream("D:\\YeosongYoon\\WebProgramming\\Source\\06_jsp\\ch14_fileUp\\WebContent\\fileUpFolder\\" + filename); // 소스폴더로 복사될 파일
	...
	<%}else{
		out.println("<h2>첨부 안 함 : filename="+filename + " / originalFilename="+originalFilename + "</h2>");
		}
	%>
</body>
</html>